...
#if defined(OTRSP)
char        in_buf[200];  // UART input buffer
...
// Handle the queries
    ...
    if (COMPARE("AUX1")) {
        int tmp_aux1 = atoi((char *)&in_buf[AFTER("AUXn")]) & 15;
          ...
          Serial.println(port[4][1]);
        #endif
        if ( (tmp_aux1 != port[4][1]) && (!( (tmp_aux1 == 4) && (port[5][1] == 5) )) && (!( (tmp_aux1 == 5) && (port[5][1] == 4) ))) {
          port[4][1] = tmp_aux1;
          #if defined(OTRSP_DEBUG)
    ...
    if (COMPARE("AUX2")) {
        int tmp_aux2 = atoi((char *)&in_buf[AFTER("AUXn")]) & 15;
          ...
          Serial.println(port[5][1]);
        #endif
        if ( (tmp_aux2 != port[5][1]) && (!( (tmp_aux2 == 4) && (port[4][1] == 5) )) && (!( (tmp_aux2 == 5) && (port[4][1] == 4) ))) {
          port[5][1] = tmp_aux2;
          #if defined(OTRSP_DEBUG)
            Serial.print("Debug: Changing port[5][1]: ");
            Serial.println(port[5][1]);
          #endif
          /* tx(port[5][0], 1);
          #if defined(OTRSP_DEBUG)
  ...
  while (Serial.available()) {
    ...
    if (inChar == '\n' || inChar == '\r') {
      ...
      if (stringComplete) {
        ...
        // Unknown query command
        if (!QCOMPARE("NAME") && !QCOMPARE("AUX1") && !QCOMPARE("AUX2")) {
          Serial.print(in_buf + '\r');
        }
